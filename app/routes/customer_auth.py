...
from app.models.models import OrderSession, SessionStatus
...
async def register_customer(request: CustomerRegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    # 2. Generate 1-Hour Customer JWT Token directly
    token_data = {
    ...
    logger.info(f"[CUSTOMER] Registered: {request.phone_number} ({request.name})")
