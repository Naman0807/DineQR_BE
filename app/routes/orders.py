...
from app.models.models import (
    Order, OrderItem, OrderSession, MenuItem, Table,
...
async def get_active_orders(db: AsyncSession = Depends(get_db)):
    logger.api_request(SERVICE, "GET", "/active")
    result = await db.execute(
        select(Order)
        .options(
            selectinload(Order.items).selectinload(OrderItem.menu_item),
...
async def update_order_status(order_id: str, update: OrderUpdate, db: AsyncSession = Depends(get_db)):
